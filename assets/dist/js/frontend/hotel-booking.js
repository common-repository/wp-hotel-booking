...
/*!*********************************************!*\
  !*** ./assets/js/frontend/hotel-booking.js ***!
  \*********************************************/
(function ($) {
  var $doc = $(document);
    ...
    hb_remove_cart_item_callback: function (cart_id, res) {
      ...
      for (var i = 0; i < cart_table.length; i++) {
        ...
        for (var y = 0; y < tr.length; y++) {
          var _tr = $(tr[y]);
          cart_item_id = _tr.attr('data-cart-id'), parent_item_id = _tr.attr('data-parent-id');
          if (cart_id === cart_item_id || cart_id === parent_item_id) {
            _tr.remove();
            continue;
          }
        }
        if (typeof res.sub_total !== 'undefined') _table.find('span.hb_sub_total_value').html(res.sub_total);
        if (typeof res.grand_total !== 'undefined') _table.find('span.hb_grand_total_value').html(res.grand_total);
        if (typeof res.advance_payment !== 'undefined') _table.find('span.hb_advance_payment_value').html(res.advance_payment);
      }
    },
    ...
    remove_cart: function () {
      ...
      $(document).on('click', '.hb_remove_cart_item', function (e) {
        ...
        $.ajax({
          ...
          res = parseJSON(res);
          if (typeof res.status === 'undefined' || res.status !== 'success') alert(hotel_booking_i18n.waring.try_again);

          // update woo cart when remove room from cart
          $('body').trigger('hb_removed_item_to_cart');
          if (typeof res.sub_total !== 'undefined') $('span.hb_sub_total_value').html(res.sub_total);
          if (typeof res.grand_total !== 'undefined') $('span.hb_grand_total_value').html(res.grand_total);
          if (typeof res.advance_payment !== 'undefined') $('span.hb_advance_payment_value').html(res.advance_payment);
